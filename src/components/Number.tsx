...
export const Number: FC<NumberProps> = ({ name, value }) => {
  ...
  return (
    <div className="mml-number">
      ...
      <span className="mml-btn mml--floating mml-btn--icon mml-number__dec" onClick={() => setState(state - 1)}>
        <Icon name="remove" />
      </span>
      <span className="mml-number__count">{state}</span>
      <span className="mml-btn mml--floating mml-btn--icon mml-number__inc" onClick={() => setState(state + 1)}>
        <Icon name="add" />
      </span>
    </div>
  );
